---
{"dg-publish":true,"permalink":"/10-system/10-apprentices-srd/","tags":["rules","quick-reference","srd","magic"],"dgShowBacklinks":true,"dgShowInlineTitle":true,"dgShowFileTree":true,"dgEnableSearch":true,"dgShowToc":true,"dgShowTags":true,"noteIcon":""}
---
...
2. **Attributes:** There are 16 attributes in a 4x4 Grid (Max 3 at creation).
   - **Magical:** Base **1 dot**. You have **2 Points** to spend.
   - **Mundane:** Base **0 dots**. You have **2 Points** to spend.
...
6. **Derived Stats:**
   ...
   - **Mastered Spells:** `Memory` rating. (Grants auto-success/free modifiers, lower complexity step by 1). You can swap your currently Mastered spells from your Grimoire outside of combat/encounters.

---

## 2. Core Resolution Engine
* **The Roll:** `2d6 + Attribute` 
* **The Casting Roll:** `2d6 + Willpower` (Destructive/Raw Energy) or `2d6 + Finesse` (Precision/Defensive)
* **Standard Outcomes:** 
  * **10+ (Strong Hit):** You succeed completely. When casting, pick 2 Overcharge options (e.g., Maximize effect, +1 damage, duplicate effect, apply status).
  * **7-9 (Mixed Success):** You succeed with a complication (e.g., Mana friction, collateral damage, exposed position).
  * **6- (Miss):** Things go wrong. The GM makes a Hard Move.
* **Defending:** Passive defense numbers (like TNs, AC, or Mind Defense) do not exist. To defend against a physical or mental attack, respond fictionally and roll the **Defend** move. You may also use an instant Reaction (like a Mastered `Scht` spell) to trigger the **Deflect Magic** move instead.
